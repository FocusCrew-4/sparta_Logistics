package com.keepgoing.order.domain;

import static org.junit.jupiter.api.Assertions.*;

import java.time.LocalDateTime;
import java.util.UUID;
import org.assertj.core.api.Assertions;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

class OrderTest {

    @DisplayName("주문 생성할 때, supplier가 null이라면 IllegalArgumentException 발생")
    @Test
    void createWithIllegalArgumentException() {
        // given
        Store supplier = null;

        Store receiver = new Store(
            UUID.fromString("7027b9af-2c0c-4f04-b04d-6d2e27bd1928"),
            "김이 필요한 집"
        );

        Product product = new Product(
            UUID.fromString("b8e2e021-2e59-4c8f-9c17-a138e8e61c9c"),
            "맛있는 김"
        );
        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            LocalDateTime.of(2025, 11, 6, 12, 0, 0),
            "맛있는 김으로 배송해주세요."
        );

        // when // then
        Assertions.assertThatThrownBy(
                () -> Order.create(
                    supplier,
                    receiver,
                    product,
                    10,
                    100000,
                    now,
                    deliveryRequestNote
                ))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("공급 업체는 필수입니다.");
    }

    @DisplayName("주문 생성할 때, receiver null이라면 IllegalArgumentException 발생")
    @Test
    void createWithIllegalArgumentException2() {
        // given
        Store supplier = new Store(
            UUID.fromString("4e88649d-37ae-4b0f-b2a2-8b79e6912a9c"),
            "김이 맛있는 집"
        );

        Store receiver = null;

        Product product = new Product(
            UUID.fromString("b8e2e021-2e59-4c8f-9c17-a138e8e61c9c"),
            "맛있는 김"
        );
        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            LocalDateTime.of(2025, 11, 6, 12, 0, 0),
            "맛있는 김으로 배송해주세요."
        );

        // when // then
        Assertions.assertThatThrownBy(
                () -> Order.create(
                    supplier,
                    receiver,
                    product,
                    10,
                    100000,
                    now,
                    deliveryRequestNote
                ))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("수령 업체는 필수입니다.");
    }

    @DisplayName("주문 생성할 때, product가 null이라면 IllegalArgumentException 발생")
    @Test
    void createWithIllegalArgumentException3() {
        // given
        Store supplier = new Store(
            UUID.fromString("4e88649d-37ae-4b0f-b2a2-8b79e6912a9c"),
            "김이 맛있는 집"
        );

        Store receiver = new Store(
            UUID.fromString("7027b9af-2c0c-4f04-b04d-6d2e27bd1928"),
            "김이 필요한 집"
        );

        Product product = null;
        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            LocalDateTime.of(2025, 11, 6, 12, 0, 0),
            "맛있는 김으로 배송해주세요."
        );

        // when // then
        Assertions.assertThatThrownBy(
                () -> Order.create(
                    supplier,
                    receiver,
                    product,
                    10,
                    100000,
                    now,
                    deliveryRequestNote
                ))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("주문 상품은 필수입니다.");
    }

    @DisplayName("주문 생성할 때, 주문 수량이 1보다 작은 경우 IllegalArgumentException 발생")
    @Test
    void createWithIllegalArgumentException4() {
        // given
        Store supplier = new Store(
            UUID.fromString("4e88649d-37ae-4b0f-b2a2-8b79e6912a9c"),
            "김이 맛있는 집"
        );

        Store receiver = new Store(
            UUID.fromString("7027b9af-2c0c-4f04-b04d-6d2e27bd1928"),
            "김이 필요한 집"
        );

        Product product = new Product(
            UUID.fromString("b8e2e021-2e59-4c8f-9c17-a138e8e61c9c"),
            "맛있는 김"
        );

        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            LocalDateTime.of(2025, 11, 6, 12, 0, 0),
            "맛있는 김으로 배송해주세요."
        );

        // when // then
        Assertions.assertThatThrownBy(
                () -> Order.create(
                    supplier,
                    receiver,
                    product,
                    0,
                    100000,
                    now,
                    deliveryRequestNote
                ))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("주문 수량은 1개 이상이어야 합니다.");
    }

    @DisplayName("주문 생성할 때, 주문 금액이 음수인 경우 IllegalArgumentException 발생")
    @Test
    void createWithIllegalArgumentException5() {
        // given
        Store supplier = new Store(
            UUID.fromString("4e88649d-37ae-4b0f-b2a2-8b79e6912a9c"),
            "김이 맛있는 집"
        );

        Store receiver = new Store(
            UUID.fromString("7027b9af-2c0c-4f04-b04d-6d2e27bd1928"),
            "김이 필요한 집"
        );

        Product product = new Product(
            UUID.fromString("b8e2e021-2e59-4c8f-9c17-a138e8e61c9c"),
            "맛있는 김"
        );

        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            LocalDateTime.of(2025, 11, 6, 12, 0, 0),
            "맛있는 김으로 배송해주세요."
        );

        // when // then
        Assertions.assertThatThrownBy(
                () -> Order.create(
                    supplier,
                    receiver,
                    product,
                    1,
                    -1,
                    now,
                    deliveryRequestNote
                ))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("주문 금액은 음수가 될 수 없습니다.");
    }

    @DisplayName("주문 생성할 때, 납부 일자가 null인 경우 IllegalArgumentException 발생")
    @Test
    void createWithIllegalArgumentException6() {
        // given
        Store supplier = new Store(
            UUID.fromString("4e88649d-37ae-4b0f-b2a2-8b79e6912a9c"),
            "김이 맛있는 집"
        );

        Store receiver = new Store(
            UUID.fromString("7027b9af-2c0c-4f04-b04d-6d2e27bd1928"),
            "김이 필요한 집"
        );

        Product product = new Product(
            UUID.fromString("b8e2e021-2e59-4c8f-9c17-a138e8e61c9c"),
            "맛있는 김"
        );

        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            null,
            "맛있는 김으로 배송해주세요."
        );

        // when // then
        Assertions.assertThatThrownBy(
                () -> Order.create(
                    supplier,
                    receiver,
                    product,
                    1,
                    10000,
                    now,
                    deliveryRequestNote
                ))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("납품 기간은 필수이며 주문 시간보다 이전일 수 없습니다.");
    }

    @DisplayName("주문 생성할 때, 납부 일자가 현재 시간보다 이전인 경우 IllegalArgumentException 발생")
    @Test
    void createWithIllegalArgumentException7() {
        // given
        Store supplier = new Store(
            UUID.fromString("4e88649d-37ae-4b0f-b2a2-8b79e6912a9c"),
            "김이 맛있는 집"
        );

        Store receiver = new Store(
            UUID.fromString("7027b9af-2c0c-4f04-b04d-6d2e27bd1928"),
            "김이 필요한 집"
        );

        Product product = new Product(
            UUID.fromString("b8e2e021-2e59-4c8f-9c17-a138e8e61c9c"),
            "맛있는 김"
        );

        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            LocalDateTime.of(2025, 11, 5, 11, 0, 0),
            "맛있는 김으로 배송해주세요."
        );

        // when // then
        Assertions.assertThatThrownBy(
                () -> Order.create(
                    supplier,
                    receiver,
                    product,
                    1,
                    0,
                    now,
                    deliveryRequestNote
                ))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("납품 기간은 필수이며 주문 시간보다 이전일 수 없습니다.");
    }

    @DisplayName("주문 생성할 때, createAt, orderedAt은 입력으로 들어온 값으로 설정된다.")
    @Test
    void create() {
        // given
        Store supplier = new Store(
            UUID.fromString("4e88649d-37ae-4b0f-b2a2-8b79e6912a9c"),
            "김이 맛있는 집"
        );

        Store receiver = new Store(
            UUID.fromString("7027b9af-2c0c-4f04-b04d-6d2e27bd1928"),
            "김이 필요한 집"
        );

        Product product = new Product(
            UUID.fromString("b8e2e021-2e59-4c8f-9c17-a138e8e61c9c"),
            "맛있는 김"
        );

        LocalDateTime now = LocalDateTime.of(2025, 11, 5, 12, 0, 0);

        DeliveryRequestNote deliveryRequestNote = new DeliveryRequestNote(
            LocalDateTime.of(2025, 11, 6, 12, 0, 0),
            "맛있는 김으로 배송해주세요."
        );

        // when
        Order order = Order.create(
            supplier,
            receiver,
            product,
            1,
            100000,
            now,
            deliveryRequestNote
        );

        // then
        Assertions.assertThat(order.getCreatedAt()).isEqualTo(now);
        Assertions.assertThat(order.getOrderedAt()).isEqualTo(now);
    }

}